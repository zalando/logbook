...
class SecurityStrategyTest {
    ...
    private final MockHttpResponse response = MockHttpResponse.create();
    ...
    void shouldNotWriteAnythingWhenSuccessfullyAuthenticatedAndAuthorized(int status) throws IOException {
    ...
    void shouldLogBothWhenForbidden(final int status) throws IOException {
        ...
        verify(sink).writeBoth(any(), any(), any());
    }
