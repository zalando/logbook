...
public class HttpResponseTest {
    ...
    public void testKnownCodes() {
        ...
        for(int i = 0; i < codes.length; i++) {
            when(response.getStatus()).thenReturn(codes[i]);
            assertNotNull(response.getReasonPhrase(), "No reason phrase for status code " + codes[i]);
        }
    ...
    public void testEmptyCodes() {
        ...
        List<Integer> list = Arrays.stream(codes).boxed().collect(Collectors.toList());
